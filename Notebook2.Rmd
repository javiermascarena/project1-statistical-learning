...
Where here X represents the data matrix, Z1 the first components and a_1 a vector of loadings that shows the direction of the original feature map.
...
Wjere S represents the covariance matrix of the data and a1 will be a unit vector so it is not affected by the scaling of variables. Scaling is important in PCA as we want to center every variables, because if not elements with a higher scale will have a larger variance affecting our analysis.
...
With this formula and the previous taking into account that a1 is a unit vector, we will obtain that the maximum variance for the first component will be its eigenvalue. The direction will be given by a1, its eigenvector. This exact procedure will be the same for the rest of components. As two uncorrelated variables have covariance 0, the next component will be orthogonal. The matrix with the principal components score will be given by:

$$
Z = X A_r
$$

Where Z is the matrix of the principal components, X the data matrix and Ar the principal component loadings.
...
To select the number of clusters we will use the Kaiser Criterion.

This criterion states that only the principal components with eigenvalues higher than 1 should appear in the analysis. It is of value greater than 1 as since each eigenvalue represents the variance contributed, a value lower than 1 would mean it explains less than the variables, so it is not useful.
...
In preprocessing we eliminated most of the variables to avoid high dimensionality and future problems. Now we want to test doing a PCA in all the variables in order to preprocess everything before doing the clustering. Before merging we still have to eliminate some variables which are in multiple datasets but not all of them, we will also have to rename most variables are the name is really similar in different datasets.
...
defense_stats <- defense_stats %>% 
  rename(TacklesDone = Tkl, SuccessfulTackles = TklW, TacklesANDIntercep = Tkl.Int,
         Clearances = Clr, PercDribblersTackled = Tkl., TacklesOwnHalf = Def.3rd,TacklesMidHalf = Mid.3rd, TacklesOppositeHalf = Att.3rd, Interceptions = Int,DribblersTackled = Tkl.1,TotalTacklesAttempts = Att, PercSuccTackles = Tkl., MissedTackles = Lost, BlockedShots = Sh, BlockedPasses = Pass, Errors = Err)
...
gca_stats <- gca_stats %>% 
  rename(ShotCreatingActions = SCA,ShotCreatingActions90 = SCA90,PassLToSCA = PassLive, PassDToSCA = PassDead,DribblesToSCA = TO,
         ShotsToSCA = Sh,FoulsDrawnToSCA = Fld,DefenseToSCA = Def,GoalCreatingActions = GCA, GoalCreatingActions90 = GCA90, PassLToGCA = PassLive.1, PassDToGCA = PassDead.1,
         DribblesToGCA = TO.1, ShotsToGCA = Sh.1,FoulsDrawnToSCA = Fld.1, DefenseToGCA = Def.1)
