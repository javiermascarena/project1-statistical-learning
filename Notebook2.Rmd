...
```{r}
final_kmeans <- kmeans(pca_data, centers = 4, nstart = 1000)
...
```{r}
library(dbscan)
kNNdistplot(scaled_data, k = 3) # Choose k based on MinPts
abline(h = 4.7, col = "red", lty = 2) # Mark your current eps
```

```{r}
library("fpc")

dbscan_cluster <- dbscan::dbscan(scaled_data, eps = 5, minPts = 3)

cluster_2_indices <- which(dbscan_cluster$cluster == 2)
pca_data_no_gk[cluster_2_indices, ]
                                 

fviz_cluster(object = dbscan_cluster, data = scaled_data, stand = FALSE,
