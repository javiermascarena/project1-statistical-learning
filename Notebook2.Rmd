...
Most of these plots are left skewed, one reason for this is the number of goalkeepers as they do not have any value in this variables. For goalkeeper stats it would happen exactly the same. Now we will use all numeric variables and see the relations and information we can obtain.
...
These boxplots clearly show that we will need to scale the data before performing any algorithm on them.

But before that we can also see that the data is mostly left skewed, to fix this we can apply a root to the numeric columns.
...
Let's look at the correlation between the different variables, to see if we can obtain some insights before running any algorithm.
...
```{r}
model <- mediate(
  Goals ~  Value... + X90s + Carries + Assists,
  data   = scaled_data,
  n.iter = 100,
  plot   = FALSE
)   
