...
We can also see that the dataset is already clean with no NA values but looking at the different columns and variables there are some columns which do not provide much information, are already in the previous dataset or are not of our interest for our current project, therefore, we will remove them. Since the merging will be done by name it is the only column present in other datasets that we will maintain. The name in the fifa dataset has numbers before which can lead to errors when merging as well, so we will have to remove it.
...
```{r}
ggplot(full_data) +
  aes(x = X90s)+  # X90S ESTA MAL PQ EL MAXIMO ANTES ERAN ALREDEDOR DE 90-80
...
plot <- ggplot(full_data)+
  ...
  geom_text_repel(
    data = subset(full_data, Assists + Goals > 20), 
    aes(label = Player),
    size = 2
  ) +
  geom_abline()+
...
plot <- ggplot(full_data)+
  ...
  geom_text_repel(
    data = subset(full_data, FoulsDrawn > 60), 
    aes(label = Player),
    size = 2
  ) +
  ggtitle("Percentage of Succesful Dribbles vs Fouls Drawn")+
  theme(plot.title = element_text(hjust = 0.5))
...
```{r}
plot <- ggplot(full_data, aes(x=CompletedPasses/AttemptedPasses, y=PassingDistance)) +
  stat_density_2d(aes(fill = ..level..), geom = "polygon", colour="white")+
  ggtitle("Percentage of Succesful Passes vs Passing Distance")+
  theme(plot.title = element_text(hjust = 0.5))
...
```{r}
plot <- ggplot(full_data)+
  aes(x = GoalsAllowed, y = ShotsOnTargetAgainst, color = Pos)+
  geom_point(alpha = 0.85)+
  geom_text_repel(
    data = subset(full_data, GoalsAllowed > 60), 
    aes(label = Player),
    size = 2
  ) +
  ggtitle("Goals Conceded vs Shot on Target against Keeper")+
  theme(plot.title = element_text(hjust = 0.5))
