...
```{r}
sum(duplicated(data_stats$Player))
```

Although there are no rows with missing values, we checked for duplicate players and  discovered that there are some players that appear more than once. This might be because maybe they played a couple of games for one team and then got transferred or loaned to another one.
...
For this we have first to convert some characters like "é" into standard English alphabet ("e") because of differences between both datasets.
...
# Handle additional characters manually for specific languages (Polish, Romanian)
unique_stats$Player <- gsub("ć|č", "c", unique_stats$Player)
unique_stats$Player <- gsub("ł", "l", unique_stats$Player)
unique_stats$Player <- gsub("ń", "n", unique_stats$Player)
unique_stats$Player <- gsub("ă", "a", unique_stats$Player)
unique_stats$Player <- gsub("ș|ş", "s", unique_stats$Player)
unique_stats$Player <- gsub("ž|ź", "z", unique_stats$Player)
unique_stats$Player <- gsub("[^[:alnum:] ]", "", unique_stats$Player)  # Removes other symbols
...
The percentage columns returned numbers between 0 and 100 but we preferred having numbers between 0 and 1 
for better understanding and simplicity

```{r}
full_data$SoT. = full_data$SoT.*0.01
full_data$PasTotCmp. = full_data$PasTotCmp.*0.01
full_data$PasShoCmp. = full_data$PasShoCmp.*0.01
